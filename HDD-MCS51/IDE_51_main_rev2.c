...
main()
{
	...
	rep_all_reg ();	 
 	//AtaWriteRegister(regATA_SECNR, 0X10);
	AtaReadRegister(regATA_SECNR); 
	AtaWriteRegister(regATA_SECNR, 5);
	AtaReadRegister(regATA_SECNR); 
...
int AtaQueryDevice(void)
{
  ...
  iNumDev = 0;
	printf("Entry: int AtaQueryDevice(void)\r\n");
  ...
  printf("Exit: int AtaQueryDevice(void) return: %d \r\n",iNumDev);
  return iNumDev;
}
...
int AtaResetAdapter(void)
{
  ...
  unsigned char ucDevCtrl;
	printf("Entry: int AtaResetAdapter(void)\r\n");

  // setup register values
  ucDevCtrl = regATA_DEVCTRL_HD15 | regATA_DEVCTRL_NIEN;
  printf("Set and then reset the soft reset bit...\r\n ");
  ...
  while (1)
  {
   ...
   printf("Exit: int AtaQueryDevice(void) return: 0 \r\n");
...
void WRITE_ATA_REG(unsigned char breg, unsigned char value)
{
	...
	while(!(READ_ATA_REG(STATUS_REG) & 0x40));
	P1 = breg;
	delay(10);
	WRn = 0;
 	P0 = value;
